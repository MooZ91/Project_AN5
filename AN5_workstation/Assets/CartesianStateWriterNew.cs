...
public class CartesianStateWriterNew : MonoBehaviour
{
    ...
    public void ReceiveJointPositions(string message)
    {
        ...
        {
            // float.TryParse("NaN", ...) succeeds and yields float.NaN instead of
            // failing -- MATLAB's inverse_kinematics.m publishes literal
            // "NaN,NaN,NaN,NaN,NaN,NaN" (not "Fuera de rango") for an
            // unreachable/unsolved pose, so that string alone isn't enough to catch
            // it. Without this check a NaN reached jointStateWriters[i].Write() ->
            // Transform.localEulerAngles, which Unity rejects at the native level.
            if (!float.TryParse(parts[i], NumberStyles.Float, CultureInfo.InvariantCulture, out ikResult[i])
                || float.IsNaN(ikResult[i]))
            {
                coordinatesDisplay.text = "Posiciones fuera del rango (IK)";
                awaitingInverseKinematics = false;
                Debug.LogWarning($"IK: resultado invalido/NaN en la posicion {i}: '{parts[i]}' (mensaje completo: '{message}').");
                return;
            }
        }
