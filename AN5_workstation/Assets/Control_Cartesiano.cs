...
{
    private const float IkPosToleranceMm = 1f;
    private const float IkRotToleranceDeg = 1f;
    ...
    private void SendCoordinates(float[] coords)
    {
        ...
        float[] q = RobotKinematics.MgiAn5(T, null, out float posErrorMm, out float rotErrorDeg);

        if (posErrorMm > IkPosToleranceMm || rotErrorDeg > IkRotToleranceDeg)
        {
            Debug.LogError($"Coordenada inalcanzable (IK no convergió: error posición {posErrorMm:F1}mm, " +
                            $"error orientación {rotErrorDeg:F1}deg). No se mueve el robot.");
            return;
        }
