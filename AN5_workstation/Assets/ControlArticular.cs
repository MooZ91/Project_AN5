...
public class ControlArticular : MonoBehaviour
{
    ...
    // Guard against slider callbacks triggered by programmatic updates (e.g. from ROS2 subscriber)
    private bool _programmaticSliderUpdate = false;
    ...
    {
        jointSliders[0].minValue = -174f;
        jointSliders[0].maxValue = 174f;
    ...
    {
        // Ignore callbacks triggered by UpdateInitialJointPositions or any programmatic update
        if (_programmaticSliderUpdate) return;
        ...
        // SetValueWithoutNotify, no "value =": este callback ya se está ejecutando
        // como reacción a un cambio del slider (manual o programático, ej. desde
        // SecCartInputController.ApplyJointAngles); reasignar con "value =" volvía a
        // disparar este mismo evento, y con valores de alta precisión o fuera de
        // rango (los que llegan de MATLAB) esa re-entrada no siempre converge --
        // causó un desborde de pila real que tiró abajo el Editor.
        jointSliders[index].SetValueWithoutNotify(roundedValue); // Actualizar slider con valor redondeado
    ...
    private void OnSpeedSliderValueChanged(float value)
    {
        ...
        speedSlider.SetValueWithoutNotify(speed); // Actualizar slider con valor redondeado, sin re-disparar el evento
        if (speedValueInput != null)
        {
            speedValueInput.text = speed.ToString();
    ...
    {
        // Once the user starts manually adjusting a joint, stop syncing the sliders
        // from live ROS feedback: the robot hasn't actually moved yet (dragging alone
        // sends no command until "Send" is pressed), so every incoming
        // current_joint_position message was snapping the slider back to the robot's
        // unchanged real position mid-drag -- indistinguishable from the slider being
        // locked. Resumes once isArticularModeActive is cleared (Send finishes).
        if (isArticularModeActive)
            return;
